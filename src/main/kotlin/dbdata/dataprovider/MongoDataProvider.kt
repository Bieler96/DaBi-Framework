...
class MongoDataProvider<T : Entity<String>>(
	...
	override suspend fun save(entity: T): T {
		...
		if (entity.id == null) {
			// Insert new entity
			(entity as Entity<String>).createdAt = now
			(entity as Entity<String>).updatedAt = now
	...
	override suspend fun findByQuerySpec(
		...
		projectionClass?.let {
			if (it != entityClass) {
				val projection =
					Projections.fields(*it.constructors.first().parameters.map { param -> Projections.include(param.name) }
						.toTypedArray())
	...
	override suspend fun aggregate(queryInfo: QueryInfo, parameters: List<Any>): Any? {
		...
		val aggregationFunction =
			queryInfo.aggregationFunction ?: throw IllegalArgumentException("Aggregation function cannot be null")
		...
		val filters = querySpec.conditions.map { condition ->
			...
			when (condition.operator) {
				...
				QueryOperator.BETWEEN -> {
					...
					Filters.and(
						Filters.gte(condition.property, values[0] as Any),
						Filters.lte(condition.property, values[1] as Any)
					)
				}
